...
public class CsController {
	...
	public ModelAndView cswriteForm(CsVo csVo) {
	...
	public ModelAndView cswrite(@ModelAttribute CsVo csVo,
			@RequestParam("customer_service_image_name") MultipartFile[] files, HttpServletRequest request) {
		// 업로드 경로 설정
		...
		map.put("userProfile", csVo); // 필요 시 추가 정보 전달

		FileImage.save(map, files);

		System.out.println("csVo=" + csVo);
	...
	public ModelAndView mycslist() {
		// 나의 문의내역 List
		List<CsVo> mycsList = csMapper.getmycsList();
